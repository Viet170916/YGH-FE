  ...
  function UploadImage( props: IProps ){
    const handleFileChange = ( event: ChangeEvent<HTMLInputElement> ) => {
      if( event.target.files.length >= 1 ){
        props.setPostImages?.( prev => {
          if( prev?.length > 0 ){
            return [ ...(prev??[]), ...(Array.from( event.target.files )
                                      ?.filter( ( file ) => {
                                        return !prev?.map( img => (img?.data as File)?.name )?.includes( file.name );
                                      } )
                                      ?.map( file => ({ data: file }) ) ?? []) ];
          }else{
            return [ ...(prev??[]), ...(Array.from( event.target.files )
    ...
    return (
      <label
        className = "custum-file-upload" htmlFor = "file" style = { {} }
      >
        <div className = "icon">
        ...
        <input type = "file" id = "file" accept = "image/*" multiple onChange = { ( event ) => handleFileChange( event ) } />
      </label>
    );
  }
