...
interface IProps{
  apartmentState: [ ApartmentDTO, ( value: ApartmentDTO ) => void ];
  display: 'accommodation' | 'apartment';
}
function ApartmentImages( props: IProps ){
  const [ apartment, setApartment ] = props.apartmentState;
  const [ images, setImages ] = useState<Image[]>( [] );
  useEffect( () => {
    setImages( apartment.images?.map( ( img ) => ({ id: img.mediaId, url: img?.["url"] }) ) );
  }, [] );
  useEffect( () => {
          ...
          return (
            ...
            >
              <Card style = { { overflow: 'auto', justifyContent: 'center' } } key = { index }>
                ...
                >Cover Image</div> }
                {
                  <AfterUploadImage className = { "apartment-image" } key = { postImage.data?.name } postImage = { postImage } setPostImages = { setImages } />
                }
              </Card>
              <TextField
...
export default React.memo( ApartmentImages );
